...
function onLoadMap()
	...
	if dummy.valid then
		myCogitatorObject = createScriptObject( "AllyRobots_Panel", "Object_Cogitator01", dummy.position, dummy.orient, "Active" )
	end
	
	dummyList = getDummyList("Robots")
	for i=1,#dummyList do
		soldiers[ i ] = spawnAlly( soldierType, "Robot", dummyList[ i ].position, dummyList[ i ].orient )
